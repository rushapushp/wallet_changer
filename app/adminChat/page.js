...
export default function AdminChat() {
  ...
  const getTechSupportMessages = async () => {
    axios({
      method: "get",
      withCredentials: true,
      url: "http://localhost:3001/api/get-tech-support-message",
    })
      .then((res) => {
        if (res.status == "200") {
  ...
  const deleteTechSupportMessage = async (messages) => {
    axios({
      method: "post",
      data: {
      ...
      withCredentials: true,
      url: "http://localhost:3001/api/delete-tech-support-message",
    })
      .then((res) => {
        console.log(res);
        if (res.status == "201") {

...
export default function Register() {
  ...
  const [usernameError, setUsernameError] = useState('Придумайте себе псевдоним')
  ...
  const registerUser = () => {
    axios({
      ...
      .then((res) => {
        console.log(res);
        if (res.status == '200'){
          setRegisterNotification(res.data + ', проверьте свою почту для подтвержения аккаунта ')
        } if (res.status == '201'){
  ...
  return (
    <div className="flex flex-col justify-center items-center gap-5 ">
      ...
      <input
        ...
        onBlur={e => blurHandler(e)}
      ></input>
      {(wrongPassword && passwordError) && <div style={{color:'red', fontSize:"15px"}}>{passwordError}</div>}
      <input
        className="bg-gray-200 rounded-full px-2"
        type="password"
        name='password'
